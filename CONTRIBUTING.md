# Contributing to ServiceNow Browser Automation
...
- **Broken selectors**: ServiceNow updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real ServiceNow instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.SERVICENOW_URL}/path/to/section`);
...
When you find working selectors for ServiceNow, document them:

```javascript
// ServiceNow selectors (verified YYYY-MM-DD)
...
ServiceNow updates its UI periodically. When selectors break:
