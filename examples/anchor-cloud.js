/**
 * anchor-cloud.js — AnchorBrowser cloud example for ServiceNow
 *
...
const { login_servicenow, create_incident } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting ServiceNow automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_servicenow(page, {});
      log('Action 1 result:', data1);

      const data2 = await create_incident(page, {});
